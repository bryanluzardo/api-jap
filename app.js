...
// asegurarse de que .env se cargue
dotenv.config();
...
// GET producto mediante id (usando jwtAuth)
...
// GET múltiples productos mediante id
...
// PUT para actualizar los usuarios (usando jwtAuth)
  ...
  try {
    ...
    user.user_name =
      typeof user_name === "string" && user_name.trim() !== ""
        ...
        : user.user_name;
    user.mail =
      typeof mail === "string" && mail.trim() !== "" ? mail.trim() : user.mail;
    user.tel =
      typeof tel === "string" && tel.trim() !== "" ? tel.trim() : user.tel;
...
// GET categoría mediante su id (usando jwtAuth)
...
// GET comentario mediante id (usando jwtAuth)
app.get("/comments/:id", jwtAuth, async (req, res) => {
  ...
  try {
    const data = await fs.readFile(
      `jsons/products_comments/${id}.json`,
      "utf8"
    );
    const parseData = JSON.parse(data);
    return res.status(200).json(parseData);
...
// POST para enviar datos del login a la base de datos, donde se identifica si ese usuario ya existe,
// si no, si su contraseña es incorrecta o si faltan campos.
app.post("/login", async (req, res) => {
  try {
    ...
    if (!user_name || !user_password) {
      return res
        .status(400)
        .json({ status: "error", message: "Faltan campos" });
    }
    user_name = String(user_name).trim().normalize("NFC");
    const user = db
      .prepare(
        "SELECT id, user_name, mail, user_password FROM usuarios WHERE user_name = ? COLLATE NOCASE"
      )
      .get(user_name);
    if (!user) {
...
app.post("/sign-up", async (req, res) => {
  try {
    ...
    const existingByMail = db
      .prepare("SELECT id FROM usuarios WHERE mail = ?")
      .get(mail);
    if (existingByMail) {
      return res
        .status(409)
        .json({ status: "mail-existe", message: "El mail ya existe" });
    }
    const existingByName = db
      .prepare("SELECT id FROM usuarios WHERE user_name = ? COLLATE BINARY")
      .get(user_name);
    if (existingByName) {
      return res
        .status(409)
        .json({ status: "username-existe", message: "El username ya existe" });
    }
    ...
    try {
      ...
      if (err && err.code === "SQLITE_CONSTRAINT") {
        return res.status(409).json({
          status: "error",
          message: "Usuario o email ya existe (constraint)",
        });
      }
      throw err;
    }
...
// GET carrito correspondiente al usuario (usando jwtAuth)
app.get("/cart", jwtAuth, async (req, res) => {
  const idUsuario = req.query.id;
  if (!idUsuario)
    return res
      .status(400)
      .json({ status: "error", message: "Falta id en query" });
  try {
    const dbIds = await db
      .prepare("SELECT productos FROM carrito WHERE user_id = ?")
      .get(idUsuario);
...
// PUT para actualizar el carrito correspondiente al usuario (usando jwtAuth)
app.put("/cart", jwtAuth, async (req, res) => {
  ...
  try {
    let cart = await db
      ...
      .get(idUsuario);
    if (cart) {
      db.prepare(`UPDATE carrito SET productos = ? WHERE user_id = ?`).run(
        ids,
        idUsuario
      );
      cart = await db
...
// DELETE para eliminar usuario de la base de datos mediante su id (usando jwtAuth)
app.delete("/usuario/:id", jwtAuth, async (req, res) => {
  const id = req.params.id;
  if (!id) {
    return res
      .status(400)
      .json({ status: "error", message: "Falta id en params" });
  }
  try {
    ...
    return res
      .status(200)
      .json({ status: "ok", message: "ok", changes: result.changes });
  } catch (err) {
